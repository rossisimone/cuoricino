...
#ifndef MultiscaleModelValve0D_H
#define MultiscaleModelValve0D_H 1
...
//! MultiscaleModelValve0D - Multiscale model for Valves
/*!
 *  @author Toni Lassila
 *
 */
class MultiscaleModelValve0D: public virtual multiscaleModel_Type,
...
public:
    ...
    //! Constructor
    explicit MultiscaleModelValve0D();

    //! Destructor
    virtual ~MultiscaleModelValve0D() {}
...
private:
    ...
    MultiscaleModelValve0D ( const MultiscaleModelValve0D& model );

    MultiscaleModelValve0D& operator= ( const MultiscaleModelValve0D& model );
    ...
    Real                   M_thetaVel;         // valve leaflet angular velocity @ t=t(n+1)
    Real                   M_thetaVel_tn;      // valve leaflet angular velocity @ t=t(n)

    Real                   M_minimumOpeningAngle;
    Real                   M_maximumOpeningAngle;
...
inline multiscaleModel_Type* createMultiscaleModelValve0D()
{
    return new MultiscaleModelValve0D();
}

} // Namespace multiscale
} // Namespace LifeV

#endif /* MultiscaleModelValve0D_H */
