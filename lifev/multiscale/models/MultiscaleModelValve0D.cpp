...
MultiscaleModelValve0D::solveModel()
{
    ...
    switch ( M_bc->handler()->bc ( 1 ).bcType() )
    {
        case Current:
            ...
            M_flowRate      = M_bc->handler()->bc ( 1 ).evaluate ( M_globalData->dataTime()->time() ); // Flow rate is negated at the outflow
...
{
    return M_pressureRight + M_flowRate;
}
...
{
    if ( boundaryFlag ( boundaryID ) == 0 )
    {
        return 0; // Imposed as a boundary condition, cannot change
    }
        ...
        if ( inputFile.is_open() )
        {
            ...
            while ( std::getline ( inputFile, line, '\n' ) )
            {
                ...
                if ( std::abs ( string2number ( stringsVector[1] ) - M_globalData->dataTime()->initialTime() ) <= deltaT )
                {
                    ...
                    M_pressureRight = string2number ( stringsVector[3] );
...
MultiscaleModelValve0D::solveForPressure()
{
    ...
    if ( M_flowRate < 0.0 && A > 0.0 )
    {
        // Forward flow
        return M_pressureLeft - M_flowRate * M_flowRate / (M_flowDischargeCoefficient * A * M_flowDischargeCoefficient * A );
    }
    else if (M_flowRate > 0.0 && A > 0.0)
    {
        // Backward flow
        return M_pressureLeft + M_flowRate * M_flowRate / (M_flowDischargeCoefficient * A * M_flowDischargeCoefficient * A );
    }
...
MultiscaleModelValve0D::solveLinearModel ( bool& solveLinearSystem )
{
    ...
    switch ( M_bc->handler()->bc ( 1 ).bcType() )
    {
        case Current: // dP/dQ (setting b.c. as Q, solving for dP/dQ)

            M_tangentFlowRate = 1.;
            M_tangentPressureRight = tangentSolveForPressure();

            break;

        case Voltage: // dQ/dS

            M_tangentPressureRight = 1.;
            M_tangentFlowRate      = -tangentSolveForFlowRate(); // WHY?
...
MultiscaleModelValve0D::tangentSolveForFlowRate()
{
    ...
    if (M_pressureLeft > M_pressureRight)
    {
        // Forward flow, modified Newton step to fix convergence
        return 2 * M_flowDischargeCoefficient * A * 0.5 / ( std::sqrt(M_pressureLeft - M_pressureRight) );
    }
    else
    {
        // Backward flow, modified Newton step to fix convergence
        return 2 * M_flowDischargeCoefficient * A * 0.5 / ( std::sqrt(M_pressureRight - M_pressureLeft) );
    }
...
MultiscaleModelValve0D::tangentSolveForPressure()
{
    ...
    {
        // Forward flow
        return -2 * M_flowRate / ( M_flowDischargeCoefficient * A * M_flowDischargeCoefficient * A ) ;
    }
    else if (M_flowRate > 0.0 && A > 0.0)
    {
        // Backward flow
...
MultiscaleModelValve0D::solveForOpeningAngle()
{
    ...
    M_thetaVel     = M_thetaVel_tn     + dt * k1_F;
