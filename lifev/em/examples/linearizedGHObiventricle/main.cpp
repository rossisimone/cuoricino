...
//    vector_Type pressure (aFESpace->map(), Repeated);
    ...
    solidBC -> handler() -> addBC("Endocardium", 1, Natural, Full, *pEndo, 3);
    solidBC -> handler() -> addBC("Endocardium", 2, Natural, Full, *pEndo, 3);
    solidBC -> handler() -> addBC("Endocardium", 4, Natural, Full, *pEndo, 3);
...
//				}
			...
			if(res1 < 500.0)
      			{
			 int tempiter(2.5/ monodomain -> timeStep() );
			iter = tempiter;
			 }
			if(res1 >= 500.0 && res1 < 1000.0)
			{
			int tempiter(2.0/ monodomain -> timeStep() );
			iter = tempiter;
			}
			if(res1 >= 1000.0 && res1 < 5000.0 )
			{
			int tempiter(1.0 / monodomain -> timeStep() );
			iter = tempiter;
			}
			if(res1 >= 5000.0 && res1 < 50000.0)
			{ int tempiter(0.5 / monodomain -> timeStep() );
			 iter = tempiter;
			}
			if(res1 >= 50000.0 && res1 <= 500000.0)
			{ int tempiter(0.2 / monodomain -> timeStep() );
			iter = tempiter;
			}
			if(res1 > 500000.0 ) iter = 1;
			}
			 ...
			 if ( k % iter == 0)
			  {
					...
					{
						std::cout << "\n*****************************************************";
						std::cout << "\nWE ARE AT TIME: "<< t << ", old 1st res = " << res1;
						std::cout << "\n*****************************************************";
