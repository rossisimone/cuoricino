...
/*!
    @file
    @brief File for the definition of the expression used to rotate a diagonal matrix.
...
#ifndef EXPRESSION_VECTORS_TO_MATRIX_HPP
#define EXPRESSION_VECTORS_TO_MATRIX_HPP
...
//! class ExpressionVectorsToMatrix Class representing an interpolation in an expression.
/*!
...
class ExpressionVectorsToMatrix
    : public ExpressionBase<ExpressionVectorsToMatrix < MeshType, MapType, SpaceDim, FieldDim > >
{
public:
    ...
    typedef ExpressionBase<ExpressionVectorsToMatrix < MeshType, MapType, SpaceDim, FieldDim > > base_Type;
    ...
    ExpressionVectorsToMatrix (fespacePtr_Type fespace, const vector_Type& vector1, const vector_Type& vector2, const vector_Type& vector3)
        : base_Type(), M_fespace (fespace), M_vector1 (vector1), M_vector2 (vector2), M_vector3 (vector3) {}

    //! Copy constructor
    ExpressionVectorsToMatrix (const ExpressionVectorsToMatrix<MeshType, MapType, SpaceDim, FieldDim>& expr)
        : base_Type(), M_fespace (expr.M_fespace), M_vector1 (expr.M_vector1), M_vector2 (expr.M_vector2), M_vector3 (expr.M_vector3) {}

    //! Destructor
    ~ExpressionVectorsToMatrix() {}
    ...
    {
        out << "transform vectors to tensors[" << FieldDim << "] ";
    }
    ...
    const vector_Type vector1() const
    {
        return M_vector1;
    }

    //! Getter for the data vector
    const vector_Type vector2() const
    {
        return M_vector2;
    }
    //! Getter for the data vector
    const vector_Type vector3() const
    {
        return M_vector3;
    }
...
*/
template<typename MeshType, typename MapType, UInt SpaceDim, UInt FieldDim>
inline ExpressionVectorsToMatrix<MeshType, MapType, SpaceDim, FieldDim>
vectorsToMatrix (
...
{
    return ExpressionVectorsToMatrix<MeshType, MapType, SpaceDim, FieldDim> (fespace, vector, matrix);
}
