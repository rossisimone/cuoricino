...
/*!
 *   @file
     @brief This file contains the definition of the EvaluationVectorsToMatrix class.

     @date 06/2011
     @author Samuel Quinodoz <samuel.quinodoz@epfl.ch>
 */

#ifndef EVALUATION_VECTORS_TO_MATRIX_HPP
#define EVALUATION_VECTORS_TO_MATRIX_HPP
...
class EvaluationVectorsToMatrix
{
public:
...
private:
    ...
    EvaluationVectorsToMatrix();

    //! Copy constructor
    EvaluationVectorsToMatrix (const EvaluationVectorsToMatrix<MeshType, MapType, SpaceDim, FieldDim>& evaluation);

    //! Expression-based constructor
    explicit EvaluationVectorsToMatrix (const ExpressionVectorsToMatrix<MeshType, MapType, SpaceDim, FieldDim>& expression);
...
class EvaluationVectorsToMatrix<MeshType, MapType, 3, 1>
{
...
public:
    ...
    //! Copy constructor
    EvaluationVectorsToMatrix (const EvaluationVectorsToMatrix<MeshType, MapType, 3, 1>& evaluation)
        :
        M_fespace       ( evaluation.M_fespace),
        M_vector1        ( evaluation.M_vector, Repeated),
        M_vector2        ( evaluation.M_vector, Repeated),
        M_vector3        ( evaluation.M_vector, Repeated),
        M_quadrature    ( 0 ),
        M_currentFE     ( evaluation.M_currentFE),
        M_tensor ( evaluation.M_tensor)
    {
    ...
    //! Expression-based constructor
    explicit EvaluationVectorsToMatrix (const ExpressionVectorsToMatrix<MeshType, MapType, 3, 1>& expression)
        :
        M_fespace ( expression.fespace() ),
        M_vector1 ( expression.vector1(), Repeated ),
        M_vector2 ( expression.vector2(), Repeated ),
        M_vector3 ( expression.vector3(), Repeated ),
        M_quadrature (0),
    ...
    void update (const UInt& iElement)
    {
        ...
        for (UInt i (0); i < M_fespace->refFE().nbDof(); ++i)
        {
            for (UInt q (0); q < M_quadrature->nbQuadPt(); ++q)
            {
                ...
                u_x[q] += M_currentFE.phi (i, q) * M_vector1[globalID];
                u_y[q] += M_currentFE.phi (i, q) * M_vector2[globalID];
                u_z[q] += M_currentFE.phi (i, q) * M_vector3[globalID];
...
private:
    ...
    std::vector<return_Type> M_tensor;
};


template<typename MeshType, typename MapType >
const flag_Type
EvaluationVectorsToMatrix<MeshType, MapType, 3, 1>::
