               ...
               char** argv )
{
    ...
    M_heart_fct.reset (new ElectroFunctors ( dataFile) );
...
Heart::run()
{
...
#ifdef MONODOMAIN
    ElectroMonodomainData _data (M_heart_fct);
#else
    ElectroBidomainData _data (M_heart_fct);
#endif
...
#ifdef MONODOMAIN
    ElectroMonodomainSolver< mesh_Type > electricModel (_data, *uFESpacePtr, bcH, M_heart_fct->M_comm);
#else
    ElectroBidomainSolver< mesh_Type > electricModel (_data, *_FESpacePtr, *uFESpacePtr, bcH, M_heart_fct->M_comm);
#endif
    ...
    boost::shared_ptr< ElectroIonicSolver< mesh_Type > > ionicModel;
    if (ion_model == 1)
    {
        if (verbose)
        {
    ...
    else if (ion_model == 2)
    {
        ...
        ionicModel.reset (new ElectroLuoRudy< mesh_Type > (_dataIonic,
...
void Heart::computeRhs ( vector_Type& rhs,
                         ElectroMonodomainSolver< mesh_Type >& electricModel,
                         boost::shared_ptr< ElectroIonicSolver< mesh_Type > > ionicModel,
...
#else
void Heart::computeRhs ( vector_Type& rhs,
                         ElectroBidomainSolver< mesh_Type >& electricModel,
                         boost::shared_ptr< ElectroIonicSolver< mesh_Type > > ionicModel,
                         ElectroBidomainData& data )
{
    bool verbose = (M_heart_fct->M_comm->MyPID() == 0);
    if (verbose)
    {
