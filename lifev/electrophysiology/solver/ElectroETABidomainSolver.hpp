...
public:
	...
	inline void copyGlobalSolution(const vectorOfPtr_Type& p) {
	  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	  //This does no copy everything, right? because the last state variable is mist. 
	...
	void inline initializePotential() {
		//(*(M_globalSolution.at(0))) *= 0;
		//The inizilation is only on the real potential not at the pointers
	...
	//! Initialize the extra cellular applied current to the value k
	//REMOVED
        //void inline initializeAppliedCurrentExtra(Real k) {
	//	(*M_IappExtraPtr)*=k;
	//}
	...
	//! given a boost function initialize the applied extra cellular current
	/*void inline setAppliedCurrentFromFunctionExtra(function_Type& f,
			Real time = 0.0) {
		M_feSpacePtr->interpolate(
				static_cast<FESpace<RegionMesh<LinearTetra>, MapEpetra>::function_Type>(f),
				*M_IappExtraPtr, time);
...
/* Why to use this with preciousPotentilPtr????
template<typename Mesh, typename IonicModel>
ElectroETABidomainSolver<Mesh, IonicModel>::ElectroETABidomainSolver(
		std::string meshName, std::string meshPath, GetPot& dataFile,
	...
	setParameters();
	init(model);
	setup(meshName, meshPath, dataFile, M_ionicModelPtr->Size());VectorSmall<3> diffusionIntra,VectorSmall<3> diffusionExtra)
}*/
...
void ElectroETABidomainSolver<Mesh, IonicModel>::importSolution(
	...
	//		M_potentialTransPtr, static_cast<UInt>(0));
	//importer->addVariable(IOData_Type::ScalarField, "Variable1", M_feSpacePtr,
	//		M_potentialExtraPtr, static_cast<UInt>(0));
	int i;
