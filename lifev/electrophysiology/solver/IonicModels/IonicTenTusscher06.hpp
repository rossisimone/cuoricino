...
public:
    ...
    //parameters getters and setters
     inline const Real  getknak() const { return knak; }
     inline const Real  getKmNa() const { return KmNa; }
     inline const Real  getKmK() const { return KmK; }
     inline const Real  getknaca() const { return knaca; }
     inline const Real  getKmNai() const { return KmNai; }
     inline const Real  getKmCa() const { return KmCa; }
     inline const Real  getksat() const { return ksat; }
     inline const Real  getn() const { return n; }


     inline const Real  getKo() const { return Ko; }
     inline const Real  getCao() const { return Cao; }
     inline const Real  getNao() const { return Nao; }


     inline const Real  getBufc() const { return Bufc; }
     inline const Real  getKbufc() const { return Kbufc; }
     inline const Real  getBufsr() const { return Bufsr; }
     inline const Real  getKbufsr() const { return Kbufsr; }
     inline const Real  getBufss() const { return Bufss; }
     inline const Real  getKbufss() const { return Kbufss; }

     inline const Real  getVmaxup() const { return Vmaxup; }
     inline const Real  getKup() const { return Kup; }
     inline const Real  getVrel() const { return Vrel; }
     inline const Real  getk1_() const { return k1_; }
     inline const Real  getk2_() const { return k2_; }
     inline const Real  getk3() const { return k3; }
     inline const Real  getk4() const { return k4; }
     inline const Real  getEC() const { return EC; }
     inline const Real  getmaxsr() const { return maxsr; }
     inline const Real  getminsr() const { return minsr; }
     inline const Real  getVleak() const { return Vleak; }
     inline const Real  getVxfer() const { return Vxfer; }


     inline const Real  getpKNa() const { return pKNa; }


     inline const Real  getRTONF() const { return RTONF; }
     inline const Real  getF() const { return F; }
     inline const Real  getR() const { return R; }
     inline const Real  getT() const { return T; }


     inline const Real  getGkr() const { return Gkr; }
     inline const Real  getGks() const { return Gks; }
     inline const Real  getGK1() const { return GK1; }
     inline const Real  getGto() const { return Gto; }
     inline const Real  getGNa() const { return GNa; }
     inline const Real  getGbNa() const { return GbNa; }
     inline const Real  getGCaL() const { return GCaL; }
     inline const Real  getGbCa() const { return GbCa; }
     inline const Real  getGpCa() const { return GpCa; }
     inline const Real  getKpCa() const { return KpCa; }
     inline const Real  getGpK() const { return GpK; }


     inline const Real  getVc() const { return Vc; }
     inline const Real  getVsr() const { return Vsr; }
     inline const Real  getVss() const { return Vss; }

   inline void setknak( const Real p ) { knak = p; }
   inline void setKmNa( const Real p ) { KmNa = p; }
   inline void setKmK( const Real p ) { KmK = p; }
   inline void setknaca( const Real p ) { knaca = p; }
   inline void setKmNai( const Real p ) { KmNai = p; }
   inline void setKmCa( const Real p ) { KmCa = p; }
   inline void setksat( const Real p ) { ksat = p; }
   inline void setn( const Real p ) { n = p; }


   inline void setKo( const Real p ) { Ko = p; }
   inline void setCao( const Real p ) { Cao = p; }
   inline void setNao( const Real p ) { Nao = p; }


   inline void setBufc( const Real p ) { Bufc = p; }
   inline void setKbufc( const Real p ) { Kbufc = p; }
   inline void setBufsr( const Real p ) { Bufsr = p; }
   inline void setKbufsr( const Real p ) { Kbufsr = p; }
   inline void setBufss( const Real p ) { Bufss = p; }
   inline void setKbufss( const Real p ) { Kbufss = p; }

   inline void setVmaxup( const Real p ) { Vmaxup = p; }
   inline void setKup( const Real p ) { Kup = p; }
   inline void setVrel( const Real p ) { Vrel = p; }
   inline void setk1_( const Real p ) { k1_ = p; }
   inline void setk2_( const Real p ) { k2_ = p; }
   inline void setk3( const Real p ) { k3 = p; }
   inline void setk4( const Real p ) { k4 = p; }
   inline void setEC( const Real p ) { EC = p; }
   inline void setmaxsr( const Real p ) { maxsr = p; }
   inline void setminsr( const Real p ) { minsr = p; }
   inline void setVleak( const Real p ) { Vleak = p; }
   inline void setVxfer( const Real p ) { Vxfer = p; }


   inline void setpKNa( const Real p ) { pKNa = p; }


   inline void setF( const Real p ) { F = p; computeRTONF(); }
   inline void setR( const Real p ) { R = p; computeRTONF(); }
   inline void setT( const Real p ) { T = p; computeRTONF(); }

   inline void computeRTONF(){ RTONF = R * T / F; }

   inline void setGkr( const Real p ) { Gkr = p; }
   inline void setGks( const Real p ) { Gks = p; }
   inline void setGK1( const Real p ) { GK1 = p; }
   inline void setGto( const Real p ) { Gto = p; }
   inline void setGNa( const Real p ) { GNa = p; }
   inline void setGbNa( const Real p ) { GbNa = p; }
   inline void setGCaL( const Real p ) { GCaL = p; }
   inline void setGbCa( const Real p ) { GbCa = p; }
   inline void setGpCa( const Real p ) { GpCa = p; }
   inline void setKpCa( const Real p ) { KpCa = p; }
   inline void setGpK( const Real p ) { GpK = p; }


   inline void setVc( const Real p ) { Vc = p; }
   inline void setVsr( const Real p ) { Vsr = p; }
   inline void setVss( const Real p ) { Vss = p; }


   inline void computeinverseVcF2() { inverseVcF2 = 1. / ( 2. * Vc * F ) ; }
   inline void computeinverseVcF() { inverseVcF = 1. / ( Vc * F ); }
   inline void computeinversevssF2() { inversevssF2 = 1. / ( 2. * Vss * F ); }
   ...
   inline Real Eca(Real Cai) {  return 0.5 * RTONF * std::log( ( Cao / Cai ) ); }
   ...
   //update concentrations
	...
	{
		if ( V >= - 40.) return 0.0;
		else return	( 0.057 * std::exp( - ( V + 80. ) / 6.8 ) );
	}
	inline Real BH(Real V)
	{
		if ( V >= - 40.)
			return ( 0.77 / ( 0.13 * ( 1. + std::exp( - ( V + 10.66 ) / 11.1 ) ) ) );
		else return ( 2.7 * std::exp( 0.079 * V ) + ( 3.1e5 ) * std::exp( 0.3485 * V ) );
	}
