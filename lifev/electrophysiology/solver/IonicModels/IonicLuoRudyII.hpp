...
/*!
  @file
  @brief Ionic model based on Luo and Rudy phase II model.
  @date 03-2013
...
#ifndef IONICLUORUDYII_HPP_INCLUDED
#define IONICLUORUDYII_HPP_INCLUDED
...
class IonicLuoRudyII : public virtual ElectroIonicModel
{
...
public:
     ...
     */
    IonicLuoRudyII ( Teuchos::ParameterList& parameterList );

    /*!
     * @param IonicLuoRudyII object
     */
    IonicLuoRudyII ( const IonicLuoRudyII& model );
    //! Destructor
    virtual ~IonicLuoRudyII() {}
    ...
    IonicLuoRudyII& operator= ( const IonicLuoRudyII& model );
    ...
    inline void setG(const Real& intoOpenSt)
    {
      	this->M_f = intoOpenSt;
    }
...
}; // class IonicLuoRudyII

//! Scientific notation calculus

inline Real ee10( int const& a );

// ===================================================
//! Constructors
// ===================================================
IonicLuoRudyII::IonicLuoRudyII()    :
    super       ( 9 ),
    M_ACap      ( 1.534 * ee10( -4 )),
    M_VMyo      ( 25.84 * ee10( -6 ) ),
    M_VJsr      ( 0.12 * ee10( -6 ) ),
    ...
    M_LtrpnTot  ( 70.0 ),
    M_HtrpnTot  ( 140.0 ),
    M_kPHtrpn   ( 2.0 * ee10( 7 ) ),
    M_kNHtrpn   ( 0.066 ),
    M_kPLtrpn   ( 4.0 * ee10( 7 ) ),
    M_kNLtrpn   ( 40.0 ),
    M_CmdnTot   ( 50.0 ),
    M_CsqnTot   ( 15.0 ),
    M_KmCmdn    ( 2.38 ),
...
IonicLuoRudyII::IonicLuoRudyII ( Teuchos::ParameterList& parameterList ) :
    super       ( 9 )
{
    M_ACap      = parameterList.get ( "areaCap", 1.534 * ee10( -4 ));
    M_VMyo      = parameterList.get ( "volMyo", 25.84 * ee10( -6 ) );
    M_VJsr      = parameterList.get ( "volJSR", 0.12 * ee10( -6 ) );
    M_VNsr      = parameterList.get ( "volNSR", 2.098 * ee10( -6 ) );
    M_VSs       = parameterList.get ( "volSS", 1.485 * ee10( -9 ) );
    ...
    M_kPHtrpn   = parameterList.get ( "kpHtrpn", 2.0 * ee10( 7 ) );
    M_kNHtrpn   = parameterList.get ( "knHtrpn", 0.066 );
    M_kPLtrpn   = parameterList.get ( "kpLtrpn", 4.0 * ee10( 7 ) );
    ...
    M_v1       	= paraneterList.get ( "maxRyRPerm", 1.8 );
    M_v2        = paraneterList.get ( "leakRateConst", 5.80 * ee10(-5));
    M_v3        = paraneterList.get ( "pumpRateATPase", 1.8 );
    M_KmUp      = paraneterList.get ( "constmUp", 0.5 );
    M_tauTr     = paraneterList.get ( "timeConstNsrJsr", 34.48 );
    M_tauXFer   = paraneterList.get ( "timeConstSubMyo", 3.125 );
    M_kap       = paraneterList.get ( "kAPlus", 0.01215 );
    M_kan       = paraneterList.get ( "kANeg", 0.1425 );
    M_kbp       = paraneterList.get ( "kBPlus", 0.00405 );
    M_kbn       = paraneterList.get ( "kBNeg", 1.930 );
    M_kcp       = paraneterList.get ( "kCPlus", 0.018 );
    M_kcn       = paraneterList.get ( "kCNeg", 0.0008 );
    M_n 		= paraneterList.get ( "coopParamN", 4.0 );
    M_m         = paraneterList.get ( "coopParamM", 3.0 );
    M_f         = paraneterList.get ( "intoOpenSt", 0.3 );
    M_g         = paraneterList.get ( "outOpenSt", 2.0 );
    M_fprime    = paraneterList.get ( "intoOpenStCa", 0.0 );
    M_gprime    = paraneterList.get ( "outOpenSt2", 0.0 );
    M_a         = paraneterList.get ( "modeTParamA", 2.0 );
    M_b         = paraneterList.get ( "modeTParamB", 2.0 );
    M_omega     = paraneterList.get ( "modeTParamO", 0.01 );
    M_PCa       = paraneterList.get ( "permCa", 33.75 * ee10(-4) );
    M_PK        = paraneterList.get ( "permK", 62.5 * ee10(-7) );
    M_ICaHalf   = paraneterList.get ( "courCaHalf", -0.458 );
}

IonicLuoRudyII::IonicLuoRudyII ( const IonicLuoRudyII& model )
{
    M_ACap		= model.M_ACap;
    M_VMyo		= model.M_VMyo;
    M_VJsr		= model.M_VJsr;
...
//! Operator
// ===================================================
IonicLuoRudyII& IonicLuoRudyII::operator= ( const IonicLuoRudyII& model )
{
    M_ACap      = model.M_ACap;
    M_VMyo      = model.M_VMyo;
    M_VJsr      = model.M_VJsr;
...
void IonicLuoRudyII::computeRhs ( const std::vector<Real>&  v,
                                         std::vector<Real>& rhs )
{
...
Real IonicLuoRudyII::computeLocalPotentialRhs ( const std::vector<Real>& v, const Real& Iapp )
{
	std::vector<Real>param1 = computeLocalSubSysCaRhs(v);
	std::vector<Real>param2 = fastINa(v);
	std::vector<Real>param3 = timeDIK(v);

	return ( - ( param2[0] + param1[0] + param3[0] + timeIIK1(v) + timeplaIkp(v) +
			exINaCa(v) + pumpINaK(v) + noSpecInsCa(v) + pumpIpCa(v) + param1[1] + backICab(v) + backINab(v) )  + Iapp );
}

std::vector<Real> IonicLuoRudyII::computeLocalGatingRhs ( const std::vector<Real>& v )
{
	Real m = v[1];
	Real h = v[2];
	Real j = v[3];
	...
	std::vector<Real>param = fastINa(v);
...
std::vector<Real> IonicLuoRudyII::computeLocalSubSysCaRhs( const std::vector<Real>& v )
{
	Real V = v[0];
	Real cK = v[6];
	Real cCa = v[7];
	...
	subSysCaRHS[9] = 2 * aplha * c2 + 4 * beta * c4 + cCa3 * M_omega / pow(M_b, 3) - ( 3 * beta + alpha + gamma * pow(M_a, 3) ) * c3;
...
// Fast Na+ Current INa
std::vector<Real>  IonicLuoRudyII::fastINa( const std::vector<Real>& v )
{
	...
	if (V >= -40)
	{
		...
		fastNa[6] = 0.3 * exp( -2.535 * ee10( -7 ) * V) / ( 1 + exp( -0.1 *( V + 32.0 ) ) );
	}
	else
	{
		fastNa[3] = 0.135 * exp( ( 80 + V ) / -6.8 );
		fastNa[4] = ( -127140 * exp( 0.2444 * V) - 3.474 * ee10(-5) * exp( -0.04391 * V ) ) * ( V + 37.78 ) / ( 1 + exp ( 0.311 * ( V + 79.23 ) ) );
		fastNa[5] = 3.56 * exp( 0.079 * V ) + 3.1 * ee10(5) * exp( 0.35 * V ) ;
...
std::vector<Real>  IonicLuoRudyII::timeDIK( const std::vector<Real>& v )
{
	...
	timeDK[1] = 7.19 * ee10(-5) * ( V + 30 ) / ( 1 - exp( -0.148 * ( V + 30 ) ) );
	timeDK[2] = 1.31 * ee10(-4) * ( V + 30 ) / ( -1 + exp( 0.0687 * ( V + 30 ) ) );

	return timeDK;
}

// Time-independent K+ Current IK1
Real  IonicLuoRudyII::timeIIK1( const std::vector<Real>& v )
{
	Real V = v[0];
	Real cK = v[6];
...
Real IonicLuoRudyII::plaIKp( const std::vector<Real>& v )
{
...
Real IonicLuoRudyII::exINaCa( const std::vector<Real>& v )
{
...
// Na+/K+ pump INaK
Real IonicLuoRudyII::pumpINaK( const std::vector<Real>& v )
{
	Real V = v[0];
	Real cNa = v[5];
...
Real IonicLuoRudyII::noSpecInsCa( const std::vector<Real>& v )
{
	...
	Real courInsK = maxInsNa * ( 1 / ( 1 + pow( M_KmNsCa /cCa, 3) ) );
...
Real IonicLuoRudyII::pumpIpCa( const std::vector<Real>& v )
{
	return M_IpCa * v[7] / ( M_KmPCa * v[7]);
}

// Ca2+ background current ICab
Real IonicLuoRudyII::backICab( const std::vector<Real>& v )
{
	Real V = v[0];
	Real cCa = v[7];
...
Real IonicLuoRudyII::backINab( const std::vector<Real>& v)
{
	Real V = v[0];
	Real cNa = v[5];
...
std::vector<Real> IonicLuoRudyII::computeLocalChannelRyrRhs( const std::vector<Real>& v )
{
	Real cCaSS= v[9];
	Real fracPC1 = v[11];
	Real fracPO1 = v[12];
...
void IonicLuoRudyII::showMe()
{
    std::cout << "\n\n\t\tIonicLuoRudyII Informations\n\n";
    ...
    std::cout << "\n\t\t End of IonicLuoRudyII Informations\n\n\n";
}
...
#endif // IONICLUORUDYII_HPP_INCLUDED
