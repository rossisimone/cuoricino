...
ReactionDiffusion::run()
{
    ...
    {
        if (verbose) std::cout << "u1 and u1 : P1 " << std::flush;
        refFE_u = &feTetraP1;
    ...
    if (verbose) std::cout << "DOF per species = " << totalUDof << std::endl;
    if (verbose) std::cout << "Calling the constructor ... ";
    ...
    if (verbose) std::cout << "Calling the model constructor ... ";
  ...
  for (UInt iVol= 0; iVol< RDModel.U1FESpace().mesh()->numVolumes(); ++iVol)
    {
	  ...
	  elvec_u1.vec()[ iNode ] = u1VecRep[ig]; elvec_u2.vec()[ iNode ] = u2VecRep[ig];
        }
