...
Heart::run()
{
...
#ifdef MONODOMAIN
    HeartMonodomainData data(M_heart_fct);
#else
    HeartBidomainData data(M_heart_fct);
#endif
...
#ifdef MONODOMAIN
    HeartMonodomainSolver< mesh_Type > electricModel (data, *uFESpacePtr, bcH, M_heart_fct->M_comm);
#else
    HeartBidomainSolver< mesh_Type > electricModel (data, *_FESpacePtr, *uFESpacePtr, bcH, M_heart_fct->M_comm);
#endif
    ...
    boost::shared_ptr< HeartAlievPanfilov < mesh_Type > > ionicModel;
        ...
        if (verbose) std::cout << "Am I working or not????? surely not!!!! :) "<< std::endl<<std::flush;

        ionicModel.reset(new HeartAlievPanfilov< mesh_Type >(dataIonic,
                                                            *meshPart.meshPartition(),
                                                            *uFESpacePtr,
                                                            *M_heart_fct->M_comm));
        if (verbose) std::cout << "I did it!!!!!!!!!!!!!!!!!!!!!!!!!!!  "<< std::endl<<std::flush;
...
/*
    ...
    UInt subiter =dataIonic.M_subiter;
    Real ds = dataIonic.timeStep()/subiter;
    std::cout << "subiterations: " << subiter << ", ds: "<< ds << std::endl;
*/
/*  for ( Real time = t0 + dt ; time <= tFinal + dt / 2.; time += dt, iter++)
    {
        ...
        chrono.start();
        MPI_Barrier(MPI_COMM_WORLD);
        for( UInt iter2 = 0; iter2 < subiter ; iter2++ ){
        	ionicModel->solveIonicModel( *M_heart_fct, ds, time+(iter2+1)*ds );
        }
        rhs*=0;
        computeRhs( rhs, electricModel, ionicModel, data );
...
//        electricModel.solutionTransmembranePotential().epetraVector().MaxValue(&minu);
        ...
        *Ueptr = electricModel.solutionExtraPotential();
        ...
        ionicModel->updatePotential(electricModel.solutionTransmembranePotential());
...
//    //! u, w with repeated map
//    vector_Type uVecRep(ionicModel->getPotential(), Repeated);
//    ionicModel->updateRepeated();
//    VectorElemental elvec_Iapp( electricModel.potentialFESpace().fe().nbFEDof(), 2 ),
//    elvec_u( electricModel.potentialFESpace().fe().nbFEDof(), 1 ),
//    elvec_Iion( electricModel.potentialFESpace().fe().nbFEDof(), 1 );
...
//    rhs.globalAssemble();
    Real coeff= data.volumeSurfaceRatio()*data.membraneCapacitance()/ data.timeStep();
//    vector_Type tmpvec(ionicModel->getPotential());
//    tmpvec*=coeff;
    rhs+=electricModel.massMatrix()*coeff*ionicModel->getPotential();
    MPI_Barrier(MPI_COMM_WORLD);
    chrono.stop();
    if (verbose) std::cout << "done in " << chrono.diff() << " s." << std::endl;
}
