...
public:
    ...
    void renewLumpedParameters  (const int&    Flag, const Real & flux);

    Real force_cardium          (const Real& t);
...
private:
    ...
    Real Pin;
    Real Pout;
    Real Rext_d; //external distal resistance
    Real Rext_p; //external proximal resistance
    Real Cp; //capacity for the pressure term in Windkessel
    int BDType; //boundary condition type (explicit Resistance, Windkessel RC, RCR)
    Real Flux;
    Real Flux_old;
    Real fExt;
    Real tAppl;
